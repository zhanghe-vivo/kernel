use crate::arch::{irq, registers::esr_el1::ESR_EL1, Context};
...
#[link_section = ".text._exception"]
#[no_mangle]
extern "C" fn trap_exception(stack_frame: &mut Context) {
    let esr = ESR_EL1.get();
    let ec = (esr >> 26) & 0x3F;
    if ec == 0x15 {
        extern "C" {
...
fn show_exception(ec: u64, stack_frame: &mut Context) {
    match ec {
