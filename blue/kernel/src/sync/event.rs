...
pub unsafe extern "C" fn rt_event_init(
    ...
    let rt_obj_ptr = &mut (*event).parent.parent as *mut KObjectBase as *mut rt_object;
...
pub unsafe extern "C" fn rt_event_send(event: *mut RtEvent, set: rt_uint32_t) -> rt_err_t {
    ...
    assert!(event != null_mut());
    let obj_ptr = &mut (*event).parent.parent as *mut KObjectBase as *mut rt_object;
...
pub unsafe extern "C" fn rt_event_control(
    ...
    assert!(event != null_mut());
    assert!(
        rt_object_get_type(&mut (*event).parent.parent as *mut KObjectBase as *mut rt_object)
...
pub extern "C" fn rt_event_info() {
    ...
    let callback = |node: &ListHead| unsafe {
        ...
        if event.parent.suspend_thread.is_empty() {
            ...
            print!("{}:", event.parent.suspend_thread.size());
