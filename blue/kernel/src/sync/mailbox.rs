...
pub unsafe extern "C" fn rt_mb_init(
    ...
    rt_object_init(
        ...
        rt_bindings::rt_object_class_type_RT_Object_Class_MailBox as u32,
        name,
    );
    ...
    (*mb).msg_pool = msgpool as *mut rt_bindings::rt_ubase_t;
    (*mb).size = size as rt_bindings::rt_uint16_t;
    (*mb).entry = 0;
    (*mb).in_offset = 0;
    (*mb).out_offset = 0;

    rt_bindings::rt_list_init!(&mut (*mb).suspend_sender_thread);
...
#[cfg(feature = "RT_USING_MAILBOX")]
#[no_mangle]
pub unsafe extern "C" fn rt_mb_detach(mb: rt_bindings::rt_mailbox_t) -> rt_bindings::rt_err_t {
    assert!(mb != null_mut());
    assert!(
        rt_object_get_type(&mut (*mb).parent.parent)
            == rt_bindings::rt_object_class_type_RT_Object_Class_MailBox as u8
    );
    assert!(rt_object_is_systemobject(&mut (*mb).parent.parent) == rt_bindings::RT_TRUE as i32);
...
pub unsafe extern "C" fn rt_mb_create(
    ...
    let mb = rt_object_allocate(
        rt_bindings::rt_object_class_type_RT_Object_Class_MailBox as u32,
        name,
    ) as rt_bindings::rt_mailbox_t;

    if mb == rt_bindings::RT_NULL as rt_bindings::rt_mailbox_t {
        return mb;
    }
    ...
    (*mb).size = size as rt_bindings::rt_uint16_t;
    let ptr = rt_malloc((*mb).size as usize * mem::size_of::<rt_bindings::rt_ubase_t>())
        as *mut rt_bindings::rt_ubase_t;
    (*mb).msg_pool = ptr;
    if (*mb).msg_pool == null_mut() {
        rt_object_delete(&mut (*mb).parent.parent);
        return rt_bindings::RT_NULL as rt_bindings::rt_mailbox_t;
    }
...
#[cfg(all(feature = "RT_USING_MAILBOX", feature = "RT_USING_HEAP"))]
#[no_mangle]
pub unsafe extern "C" fn rt_mb_delete(mb: rt_bindings::rt_mailbox_t) -> rt_bindings::rt_err_t {
    assert!(mb != null_mut());
    assert!(
        rt_object_get_type(&mut (*mb).parent.parent)
            == rt_bindings::rt_object_class_type_RT_Object_Class_MailBox as u8
    );
    assert!(rt_object_is_systemobject(&mut (*mb).parent.parent) == rt_bindings::RT_FALSE as i32);
...
unsafe extern "C" fn _rt_mb_send_wait(
    ...
    rt_bindings::rt_object_hook_call!(rt_object_put_hook, &mut (*mb).parent.parent);
    ...
    *(*mb).msg_pool.offset((*mb).in_offset as isize) = value;
    ...
    //unsafemonitor
    if (*mb).entry < rt_bindings::RT_MB_ENTRY_MAX as rt_bindings::rt_uint16_t {
        (*mb).entry += 1;
    } else {
        rt_bindings::rt_hw_interrupt_enable(level);
        return -(rt_bindings::RT_EFULL as rt_bindings::rt_err_t);
...
pub unsafe extern "C" fn rt_mb_send_wait(
    ...
    _rt_mb_send_wait(mb, value, timeout, rt_bindings::RT_UNINTERRUPTIBLE as i32)
}
...
pub unsafe extern "C" fn rt_mb_send_wait_interruptible(
    ...
    _rt_mb_send_wait(mb, value, timeout, rt_bindings::RT_INTERRUPTIBLE as i32)
}
...
pub unsafe extern "C" fn rt_mb_send_wait_killable(
    ...
    _rt_mb_send_wait(mb, value, timeout, rt_bindings::RT_KILLABLE as i32)
}

#[cfg(feature = "RT_USING_MAILBOX")]
#[no_mangle]
pub unsafe extern "C" fn rt_mb_send(
    mb: rt_bindings::rt_mailbox_t,
    value: rt_bindings::rt_ubase_t,
) -> rt_bindings::rt_err_t {
...
#[cfg(feature = "RT_USING_MAILBOX")]
#[no_mangle]
pub unsafe extern "C" fn rt_mb_send_killable(
    mb: rt_bindings::rt_mailbox_t,
    value: rt_bindings::rt_ubase_t,
) -> rt_bindings::rt_err_t {
...
#[cfg(feature = "RT_USING_MAILBOX")]
#[no_mangle]
pub unsafe extern "C" fn rt_mb_urgent(
    mb: rt_bindings::rt_mailbox_t,
    value: rt_bindings::rt_ubase_t,
) -> rt_bindings::rt_err_t {
    assert!(mb != null_mut());
...
unsafe extern "C" fn _rt_mb_recv(
    ...
    let mut tick_delta = 0;
    rt_bindings::rt_object_hook_call!(rt_object_trytake_hook, &mut (*mb).parent.parent);
    ...
    *value = *(*mb).msg_pool.offset((*mb).out_offset as isize);
    ...
    if (*mb).suspend_sender_thread.is_empty() == false {
        ...
        rt_bindings::rt_object_hook_call!(rt_object_take_hook, &mut (*mb).parent.parent);
...
pub unsafe extern "C" fn rt_mb_recv(
    ...
    return _rt_mb_recv(mb, value, timeout, rt_bindings::RT_UNINTERRUPTIBLE as i32);
}
...
pub unsafe extern "C" fn rt_mb_recv_interruptible(
    ...
    return _rt_mb_recv(mb, value, timeout, rt_bindings::RT_INTERRUPTIBLE as i32);
}
...
pub unsafe extern "C" fn rt_mb_recv_killable(
    ...
    return _rt_mb_recv(mb, value, timeout, rt_bindings::RT_KILLABLE as i32);
}
...
pub struct MailBox {
    #[pin]
    mb_ptr: rt_bindings::rt_mailbox_t,
    #[pin]
    _pin: PhantomPinned,
}
...
impl MailBox {
    pub fn new(name: &str, size: usize) -> Result<Self, Error> {
        ...
        if result == rt_bindings::RT_NULL as rt_bindings::rt_mailbox_t {
    ...
    pub fn send_wait(&self, set: u32, timeout: rt_bindings::rt_int32_t) -> Result<(), Error> {
    ...
    pub fn send_interruptible(&self, set: u32) -> Result<(), Error> {
        let result =
            unsafe { rt_mb_send_interruptible(self.mb_ptr, set as rt_bindings::rt_ubase_t) };
        if result == rt_bindings::RT_EOK as rt_bindings::rt_err_t {
            Ok(())
        } else {
            Err(Error::from_errno(result))
    ...
    pub fn send_killable(&self, set: u32) -> Result<(), Error> {
        let result = unsafe { rt_mb_send_killable(self.mb_ptr, set as rt_bindings::rt_ubase_t) };
        if result == rt_bindings::RT_EOK as rt_bindings::rt_err_t {
            Ok(())
        } else {
            Err(Error::from_errno(result))
...
pub extern "C" fn rt_mailbox_info() {
    ...
    let callback = |node: &ListHead| unsafe {
        let mailbox = &*(crate::list_head_entry!(node.as_ptr(), KObjectBase, list)
            as *const rt_bindings::rt_mailbox);
        ...
        if mailbox.parent.suspend_thread.is_empty() {
            println!("{}", mailbox.parent.suspend_thread.len());
        } else {
            print!("{}:", mailbox.parent.suspend_thread.len());
