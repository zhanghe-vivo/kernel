...
macro_rules! rt_message_queue_priority {
    ($msg:expr, $mq:expr, $prio: expr) => {
        ...
        let node: *mut rt_bindings::rt_mq_message = null_mut();
        let mut prev_node: *mut rt_bindings::rt_mq_message = null_mut();
...
macro_rules! rt_message_queue_non_prio {
    ($msg:expr, $mq:expr) => {
        if (*$mq).msg_queue_tail != null_mut() {
            (*((*$mq).msg_queue_tail as *mut rt_bindings::rt_mq_message)).next = $msg
        }
...
pub unsafe extern "C" fn rt_mq_init(
    mq: rt_bindings::rt_mq_t,
    name: *const core::ffi::c_char,
    msgpool: *mut core::ffi::c_void,
    msg_size: rt_bindings::rt_size_t,
    pool_size: rt_bindings::rt_size_t,
    ...
    rt_object_init(
        ...
        rt_bindings::rt_object_class_type_RT_Object_Class_MessageQueue as u32,
        name,
    );
    ...
    let msg_align_size = align_up_size(msg_size as usize, rt_bindings::RT_ALIGN_SIZE as usize);
    (*mq).msg_size = msg_size as rt_bindings::rt_uint16_t;
    (*mq).max_msgs = (pool_size as usize
        / (msg_align_size + mem::size_of::<rt_bindings::rt_mq_message>()))
        as rt_bindings::rt_uint16_t;
    ...
    for temp in 0..(*mq).max_msgs as usize {
        let head = ((*mq).msg_pool as *mut rt_bindings::rt_uint8_t).offset(
            (temp * (msg_align_size as usize + mem::size_of::<rt_bindings::rt_mq_message>()))
                as isize,
        ) as *mut rt_bindings::rt_mq_message;
        (*head).next = (*mq).msg_queue_free as *mut rt_bindings::rt_mq_message;
        (*mq).msg_queue_free = head as *mut c_void;
    }

    (*mq).entry = 0;

    rt_bindings::rt_list_init!(&mut (*mq).suspend_sender_thread);
...
#[cfg(feature = "RT_USING_MESSAGEQUEUE")]
#[no_mangle]
pub unsafe extern "C" fn rt_mq_detach(mq: rt_bindings::rt_mq_t) -> rt_bindings::rt_err_t {
    assert!(mq != null_mut());
    assert!(
        rt_object_get_type(&mut (*mq).parent.parent)
            == rt_bindings::rt_object_class_type_RT_Object_Class_MessageQueue as u8
    );
    assert!(rt_object_is_systemobject(&mut (*mq).parent.parent) == rt_bindings::RT_TRUE as i32);
...
pub unsafe extern "C" fn rt_mq_create(
    name: *const core::ffi::c_char,
    msg_size: rt_bindings::rt_size_t,
    max_msgs: rt_bindings::rt_size_t,
    flag: rt_bindings::rt_uint8_t,
) -> rt_bindings::rt_mq_t {
    assert!(
        (flag == rt_bindings::RT_IPC_FLAG_FIFO as rt_bindings::rt_uint8_t)
    ...
    let mq = rt_object_allocate(
        rt_bindings::rt_object_class_type_RT_Object_Class_MessageQueue as u32,
        name,
    ) as *mut rt_bindings::rt_messagequeue;
    if mq == null_mut() {
        return mq;
    }
    ...
    //unsafemonitor
    (*mq).msg_size = msg_size as rt_bindings::rt_uint16_t;
    (*mq).max_msgs = max_msgs as rt_bindings::rt_uint16_t;

    (*mq).msg_pool = rt_malloc(
        (msg_align_size as usize + mem::size_of::<rt_bindings::rt_mq_message>())
...
#[cfg(all(feature = "RT_USING_MESSAGEQUEUE", feature = "RT_USING_HEAP"))]
#[no_mangle]
pub unsafe extern "C" fn rt_mq_delete(mq: rt_bindings::rt_mq_t) -> rt_bindings::rt_err_t {
    assert!(mq != null_mut());
    assert!(
        rt_object_get_type(&mut (*mq).parent.parent)
            == rt_bindings::rt_object_class_type_RT_Object_Class_MessageQueue as u8
    );
    assert!(rt_object_is_systemobject(&mut (*mq).parent.parent) == rt_bindings::RT_FALSE as i32);
...
unsafe extern "C" fn _rt_mq_send_wait(
    ...
    size: rt_bindings::rt_size_t,
    prio: rt_bindings::rt_int32_t,
    ...
    rt_bindings::rt_debug_scheduler_available!(scheduler);

    if size > (*mq).msg_size as rt_bindings::rt_size_t {
        return -(rt_bindings::RT_ERROR as rt_bindings::rt_err_t);
    }

    let mut tick_delta = 0;
    rt_bindings::rt_object_hook_call!(rt_object_put_hook, &mut (*mq).parent.parent);

    let mut level = rt_bindings::rt_hw_interrupt_disable();

    let mut msg = (*mq).msg_queue_free as *mut rt_bindings::rt_mq_message;
    ...
    while {
        msg = (*mq).msg_queue_free as *mut rt_bindings::rt_mq_message;
        msg
    } == null_mut()
    {
        ...
        if timeout > 0 {
            tick_delta = rt_bindings::rt_tick_get();
        ...
        if timeout > 0 {
            tick_delta = rt_bindings::rt_tick_get() - tick_delta;
            timeout -= tick_delta as rt_bindings::rt_int32_t;
            if timeout < 0 {
                timeout = 0;
            }
        }
    ...
    (*msg).length = size as rt_bindings::rt_ssize_t;

    rt_bindings::rt_memcpy(crate::rt_get_message_addr!(msg), buffer, size);
    ...
    if (*mq).entry < rt_bindings::RT_MQ_ENTRY_MAX as rt_bindings::rt_uint16_t {
        (*mq).entry += 1;
    } else {
        rt_bindings::rt_hw_interrupt_enable(level);
        return -(rt_bindings::RT_EFULL as rt_bindings::rt_err_t);
...
pub unsafe extern "C" fn rt_mq_urgent(
    ...
    let msg = (*mq).msg_queue_free as *mut rt_bindings::rt_mq_message;
    if msg == null_mut() {
        rt_bindings::rt_hw_interrupt_enable(level);
        return -(rt_bindings::RT_EFULL as rt_bindings::rt_err_t);
    }
    ...
    (*msg).next = (*mq).msg_queue_head as *mut rt_bindings::rt_mq_message;
    (*mq).msg_queue_head = msg as *mut c_void;
...
unsafe extern "C" fn _rt_mq_recv(
    ...
    size: rt_bindings::rt_size_t,
    prio: *mut rt_bindings::rt_int32_t,
    timeout: rt_bindings::rt_int32_t,
    suspend_flag: i32,
) -> rt_bindings::rt_ssize_t {
    let mut timeout = timeout;
    ...
    let scheduler = timeout != 0;
    rt_bindings::rt_debug_scheduler_available!(scheduler);
    rt_bindings::rt_object_hook_call!(rt_object_trytake_hook, &mut (*mq).parent.parent);
    ...
    while (*mq).entry == 0 {
        ...
        let mut tick_delta: rt_bindings::rt_uint32_t = 0;
        if timeout > 0 {
            tick_delta = rt_bindings::rt_tick_get();
    ...
    let msg = (*mq).msg_queue_head as *mut rt_bindings::rt_mq_message;
    ...
    let mut len = (*msg).length as rt_bindings::rt_size_t;

    if len > size {
        len = size;
    }

    rt_bindings::rt_memcpy(buffer, crate::rt_get_message_addr!(msg), len);
    ...
    (*msg).next = (*mq).msg_queue_free as *mut rt_bindings::rt_mq_message;
    ...
    if (*mq).suspend_sender_thread.is_empty() == false {
        ...
        rt_bindings::rt_object_hook_call!(rt_object_take_hook, &mut (*mq).parent.parent);

        rt_bindings::rt_schedule();

        return len as rt_bindings::rt_ssize_t;
    }
    ...
    pub fn new(name: &str, msg_size: usize, max_msgs: usize) -> Result<Self, Error> {
        let result = unsafe {
            rt_mq_create(
                ...
                msg_size as rt_bindings::rt_size_t,
                max_msgs as rt_bindings::rt_size_t,
    ...
    pub fn send_wait(&self, msg: *const u8, size: usize, timeout: i32) -> Result<(), Error> {
        let result = unsafe {
            rt_mq_send_wait(
                ...
                msg as *const c_void,
                size as rt_bindings::rt_size_t,
                timeout as rt_bindings::rt_int32_t,
            )
        };
        if result == rt_bindings::RT_EOK as rt_bindings::rt_err_t {
            Ok(())
...
pub extern "C" fn rt_msgqueue_info() {
    ...
    let callback = |node: &ListHead| unsafe {
        let msgqueue = &*(crate::list_head_entry!(node.as_ptr(), KObjectBase, list)
            as *const rt_bindings::rt_messagequeue);
