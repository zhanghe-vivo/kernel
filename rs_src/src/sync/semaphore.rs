...
#[no_mangle]
pub unsafe extern "C" fn rt_sem_init(sem: *mut rt_semaphore, name: *const core::ffi::c_char, value: rt_uint32_t, flag: rt_uint8_t) -> rt_err_t {

    assert!(sem != null_mut());
    assert!(value < 0x10000 as rt_uint32_t);
    assert!((flag == RT_IPC_FLAG_FIFO as rt_uint8_t) || (flag == RT_IPC_FLAG_PRIO as rt_uint8_t));
    rt_object_init(&mut ((*sem).parent.parent), rt_object_class_type_RT_Object_Class_Semaphore as u32, name);
...
pub unsafe extern "C" fn rt_sem_detach(sem: *mut rt_semaphore) -> rt_err_t{
    ...
    assert!(sem != null_mut());
    assert!(rt_object_get_type(&mut (*sem).parent.parent) == rt_object_class_type_RT_Object_Class_Semaphore as u8);
...
#[no_mangle]
pub unsafe extern "C" fn rt_sem_create(name: *const core::ffi::c_char, value: rt_uint32_t, flag : rt_uint8_t) -> rt_sem_t{

    assert!(value < 0x10000 as rt_uint32_t);
    assert!((flag == RT_IPC_FLAG_FIFO as rt_uint8_t) || (flag == RT_IPC_FLAG_PRIO as rt_uint8_t));
    rt_debug_not_in_interrupt!();
    let sem = rt_object_allocate(rt_object_class_type_RT_Object_Class_Semaphore as u32, name) as rt_sem_t;
    if sem == null_mut() {
        return sem;
    }
...
#[cfg(feature = "RT_USING_SEMAPHORE")]
#[no_mangle]
unsafe extern "C" fn _rt_sem_take(sem : rt_sem_t, timeout : rt_int32_t, suspend_flag: i32) -> rt_err_t
{
...
impl Semaphore {
    pub fn new(name: &str, value: u32) -> Result<Self, Error> {
        let result = unsafe { rt_sem_create(name.as_ptr() as *const c_char, value, RT_IPC_FLAG_PRIO as u8) };
